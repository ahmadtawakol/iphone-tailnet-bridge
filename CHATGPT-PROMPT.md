...
```text
Use the physical iPhone connected through the configured
iphone-tailnet-bridge. Do not use a simulator unless I explicitly ask for one.

1. Run `xcrun devicectl list devices` and identify the paired, available iPhone.
2. Inspect the project instructions and use the project's normal build command.
...
3. Build and sign the app for the physical iPhone.
4. Install the resulting signed .app on the identified device.
5. Launch the app on the device.
6. Report these as separate results: build, installation, and launch.
...
Do not print, expose, copy, or modify `bridge.local.conf`, authTag, pairing
records, signing credentials, or other secrets. Do not change the bridge
configuration unless I explicitly ask.
```
